...
while True:
    ...
    if results.multi_hand_landmarks: # if the hand is found
        ...
        # 왼손일 경우에, FLIP 이 되어 연산을 새로이 해야된다
        for handLms in results.multi_hand_landmarks: # for each hand
            ...
            if len(lmList) != 0: #if recognized well
                ...
                cv2.putText(img, function, (100, 60), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 3)
                cv2.putText(img, "thick :  " + str(thickness), (45, 100), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 3)
            ...
            if (flag) : # finger is not 1
                ...
                red_index += 1
                #thickpoints.append(deque(maxlen=512))
                #thick_index += 1
            ...
            #thickpoints[thick_index].appendleft(center)
        ...
        for j in range(len(points[i])):
            for k in range(1, len(points[i][j])):
                if points[i][j][k - 1] is None or points[i][j][k] is None:
                    continue
                cv2.line(img, points[i][j][k - 1], points[i][j][k], colors[i], thickness)
